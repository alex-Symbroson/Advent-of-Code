...
(setq x 0 y 0 tx x ty y sx nil sy nil cmds nil lcmd 0 lf #\.)
(defun read-fun () (loop
    (setq tx x ty y move-cmd nil)
    (when (not cmds)
        ;(loop while (equal move-cmd lcmd) do
            (setq move-cmd (code-char (shell "../getc")))
            ;(setq move-cmd (aref "wasd" (random 4))))
        (setq lcmd move-cmd)
        (push move-cmd cmds))
    (print-xy lf)
...
(with-open-file (stream "aoc-15-1.out")
